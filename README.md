...
- **Classify cross-paper disagreements (not just merges)**: reconcile also records opposing claims as typed edges, splitting them into four buckets so the high-confidence crux signal stays clean: `contradicts` (genuine cross-author dispute on the same question), `weak_contradicts` (the shared question is too abstract to overlap both nodes — a coherence-hallucination guard), `self_inconsistency` (same author disagreeing across their own documents), and `quantitative_divergence` (both agree the phenomenon exists, only the figure differs). Only `contradicts` feeds cruxes / `contested` status. Reconcile clears its own prior edges first, so re-runs are idempotent.
...
| **reconcile** | Merges cross-paper `claim`/`evidence` paraphrases (sim 0.65–0.84) into canonical nodes with union attestations. Sets `support_count`, `contradict_count`, `epistemic_status`. Also classifies cross-paper disagreements into typed edges: `contradicts` / `weak_contradicts` / `self_inconsistency` / `quantitative_divergence` (only `contradicts` feeds cruxes). Prunes junk/stance-conflicting attestations; clears its own prior edges so re-runs are idempotent. **Runs before structure.** | `episteme/prompts/reconcile.py` | `RECONCILE_PAIR_VERDICT` (Haiku for ambiguous 0.65–0.75 pairs), `RECONCILE_SHARED_QUESTION_MIN_OVERLAP` | `graph.json` (fewer nodes, multi-attestation, typed disagreement edges) | **Debate** → Cross-paper canonical |
